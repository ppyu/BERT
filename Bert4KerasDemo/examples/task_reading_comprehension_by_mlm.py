...
# 标注数据
webqa_data = json.load(open('qa_datasets/WebQA.json',encoding='utf-8'))
sogou_data = json.load(open('qa_datasets/SogouQA.json',encoding='utf-8'))

# 保存一个随机序（供划分valid用）
if not os.path.exists('tmp/random_order.json'):
    random_order = list(range(len(sogou_data)))
    np.random.shuffle(random_order)
    json.dump(random_order, open('tmp/random_order.json', 'w'), indent=4)
else:
    random_order = json.load(open('tmp/random_order.json'))
