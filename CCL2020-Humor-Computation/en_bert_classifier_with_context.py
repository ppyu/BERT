# -*- coding: utf-8 -*-
"""
@File   : en_bert_classifier_with_context.py
@Author : Pengy
@Date   : 2020/8/13
...
class HumorCompProcessor(DataProcessor):
    """Base class for data converters for sequence classification data sets."""
    ...
    def get_train_examples(self, data_dir):
        """Gets a collection of `InputExample`s for the train set."""
        return self._create_exaples(self._read_tsv(os.path.join(data_dir, "en_train.csv")), "train")
    ...
    def get_test_examples(self, data_dir):
        """Gets a collection of `InputExample`s for prediction."""
        return self._create_exaples(self._read_tsv(os.path.join(data_dir, "en_test.csv")), "test")
    ...
    def _create_exaples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if (set_type == "dev" and int(dialogue_id) in self.dev_indexs) \
                ...
                text_b = None
                text_b_len = FLAGS.max_seq_length - 3 - len(text_a.split(" "))
                if text_b_len > 0:
    ...
    def _get_text_b(self, text_b_len, dialogue_id, index, lines):
        ...
        while True:
            ...
            if line[1] == dialogue_id:
                text_splited = line[4].split(" ")
                if len(text_splited) < left_size:
                    text_b_list.insert(0, text_splited)
                    left_size -= len(text_splited)
                    left -= 1
                else:
                    text_b_list.insert(0, text_splited[-left_size:])
                    left_size -= left_size
                    left -= 1
                    break
            else:
        ...
        if right_size > 0:
            while True:
                ...
                if line[1] == dialogue_id:
                    text_splited = line[4].split(" ")
                    if len(text_splited) < right_size:
                        text_b_list.append(text_splited)
                        right_size -= len(text_splited)
                        right += 1
                    else:
                        text_b_list.append(text_splited[:right_size])
                    ...
                    break
        text_b = ""
        for sub_list in text_b_list:
            text_b += ''.join(sub_list)
        return text_b
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        output_eval_file = os.path.join(FLAGS.output_dir, "cn_eval_results.txt")
